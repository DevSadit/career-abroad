...
function isConfidentCandidate(candidate) {
  ...
  return Boolean(
    match &&
      (match.exactQuestionMatch ||
        match.exactAnswerMatch ||
        match.score >= 20 ||
        match.questionConceptOverlap >= 2 ||
        (match.questionOverlap >= 2 && match.score >= 12)),
  );
}
...
export function retrieveFaqCandidates(query, options = {}) {
  ...
  if (!cleanedQuery || cleanedQuery.length < 3) {
    return {
      ...
      suggestions: [],
      reply:
        "Could you be a bit more specific? You can ask me about applications, scholarships, SOP, visas, documents, or living costs.",
    };
  }
  ...
  const candidates = rankedEntries
    .filter(
      ({ match }) =>
        match.score > 0 || match.exactQuestionMatch || match.exactAnswerMatch,
    )
    .slice(0, 3)
    .map(({ entry, match }, index) => ({
      ...
      country: entry.countryName,
      href: entry.href,
      sourceLabel: `${entry.countryName} FAQ`,
      score: match.score,
      match,
    }));
  const bestCandidate = candidates[0] ?? null;
  const isConfident = isConfidentCandidate(bestCandidate);
...
export function findFaqAnswer(query, options = {}) {
  const searchResult = retrieveFaqCandidates(query, options);

  if (!searchResult.validQuery) {
    return {
      found: false,
      activeCountry: searchResult.activeCountry,
      reply: searchResult.reply,
      suggestions: searchResult.suggestions,
    };
  }

  if (!searchResult.isConfident || !searchResult.bestCandidate) {
    return {
      found: false,
      activeCountry: searchResult.activeCountry,
      reply:
        "I couldn't find a confident match for that. Try rephrasing or picking a specific country. If it's not covered in the FAQ, feel free to reach out directly.",
      suggestions: searchResult.suggestions,
    };
  }
  ...
  return {
    found: true,
    activeCountry: searchResult.activeCountry,
    reply: searchResult.bestCandidate.answer,
    country: searchResult.bestCandidate.country,
    matchedQuestion: searchResult.bestCandidate.question,
    sourceHref: searchResult.bestCandidate.href,
